async function fetchEthTransaction(transactionHash, retryCount = 2) {
  ...
  const raw = JSON.stringify({
    ...
    query MyQueryWithVariables($hash: [String!]) {
      EVM(dataset: combined, network: eth) {
        buyside: DEXTrades(
          where: {Block: {Date: {since: "2024-08-10"}}, TransactionStatus: {Success: true}, Transaction: {Hash: {in: $hash}}}
        ) {
          Block {
            Time
          }
  ...
  try {
      ...
      if (result.errors && result.errors.length !== 0) {
        if (retryCount > 0) {
            await delay(1000);
            return await fetchEthTransaction(transactionHash,retryCount - 1);
        }
      }
      ...
      if (!data) {
        ...
        processedData = await Promise.all(data.map(async item => {
          ...
          if (Number(item.Transaction.Value) > 0 && item.Trade.Sell.Currency.SmartContract == "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2") {
              item.Trade.Sell.Currency.Symbol = 'ETH'
          } else if(item.Trade.Buy.Currency.SmartContract == "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2") {
