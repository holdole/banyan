...
export default {
  setup() {
    ...
    const onLoad = debounce(async () => {
      try {
        const processData = async (
          tradeData,
          displayCountRef,
          fetchTransactionFunction,
          loadingRef,
          finishedRef
        ) => {
          let dataSlice = tradeData.value.slice(
          ...
          if (dexdata.length != 0) {
            tradeData.value = await mergeData(tradeData.value, dexdata);
            displayCountRef.value = displayCountRef.value + 10;
          }

          loadingRef.value = false;
          if (displayCountRef.value >= tradeData.value.length) {
    ...
    const fetchWithRetryWithFid = async (fid) => {
      const fetchOperation = async () => {
    ...
    onMounted(async () => {
      let fid = store.state.fid;
      try {
        const result = await fetchWithRetryWithFid(fid);
        
        if (result != undefined) {
          userPopup.value = result.users[0];
