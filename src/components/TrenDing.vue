<template>
  <div v-if="isLoadingTrendingData" class="loading-container">
  ...
  <div class="app-container" v-else>
    ...
    <div class="homepage-home">
      <div v-if="isLoadingTrendingData" class="loading-container-trending">
        <div class="spinner"></div>
      </div>
      <div v-if="!isLoadingTrendingData">
        <div class="banyan-filter-container">
          <span class="banyan-filter-title">Trending</span>
        ...
        <van-pull-refresh v-model="refreshing" pulling-text="Pull down to refresh..."  loosing-text="Release to refresh..." loading-text="Loading..."  @refresh="onRefresh">
          <van-list v-model:loading="loadingTrending" :finished="finishedTrending" loading-text="loading" finished-text="No More" @load="onLoad">
            <van-cell v-for="(item, index) in trendingData.slice(0, displayTrendingCount).filter(item => item.transaction)" :key="index" class="homepage-cell" @click="routeToSwapModal(item.network, item.transaction.Trade.Buy.Currency.SmartContract,item.transaction.Trade.Sell.Currency.SmartContract, item.address)">
              <template #default>
...
export default {
  ...
  setup() {
      const data = ref([]);
      const isLoadingTrendingData = ref(true)
      const trendingData = ref([]);
      const displayTrendingCount = ref(15);
      const loadingTrending = ref(false);
      const finishedTrending = ref(false);
      ...
      onMounted(async () => {
        try {
          const fetchTrendingData = async () => {
            return await axios.get('https://app.banyan.top/api/getTrendingWalletEth', {
              params: {
          ...
          const response = await retryOperation(fetchTrendingData);
          trendingData.value = await response.data;
        } catch (error) {
          console.error("Error fetching trending data after 3 attempts:", error);
        } finally {
          isLoadingTrendingData.value = false;
        }
      });
        ...
        try {
          const fetchTrendingData = async () => {
            if (filterOptions.chain == 'Ethereum') {
              ...
              return await axios.get('https://app.banyan.top/api/getTrendingWallet', {
                params: {
                  followers: filterOptions.followers,
                  amount: filterOptions.amount
                }
      ...
      const onLoad = debounce(async () => {
        try {
          const processData = async (fetchTransactionFunction) => {
            let dataSlice = trendingData.value.slice(
              displayTrendingCount.value,
              displayTrendingCount.value + 15
            );
            ...
            if (dexdata.length != 0) {
              trendingData.value = await mergeData(trendingData.value, dexdata);
              displayTrendingCount.value = displayTrendingCount.value + 15;
            }
          };

          if (filterOptions.chain == 'Ethereum') {
            await processData(fetchEthTransaction);
          } else if (filterOptions.chain == 'Base') {
            await processData(fetchBaseTransaction);
          }

          loadingTrending.value = false;
          if (displayTrendingCount.value >= trendingData.value.length) {
            finishedTrending.value = true;
          }
        } catch (error) {
          console.error("Error in trending onLoad after 3 attempts:", error);
          loadingTrending.value = false;
        }
      }, 3000);
      ...
      const handleChainTrending = async () => {
        displayTrendingCount.value = 15;
        isLoadingTrendingData.value = true;
